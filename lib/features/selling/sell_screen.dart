...
class SellScreen extends StatefulWidget {
  ...
  @override State<SellScreen> createState() => _SellScreenState();
}
class _SellScreenState extends State<SellScreen> {
  ...
  @override Widget build(BuildContext context){
    return SafeArea(child:ListView(padding:const EdgeInsets.fromLTRB(16,16,16,30),children:[
      Row(children:[const Expanded(child:Text('Vendi',style:TextStyle(fontSize:29,fontWeight:FontWeight.w900))),IconButton(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const AuthScreen())),icon:const Icon(Icons.verified_user_outlined))]),
      const Text('Pubblica il tuo annuncio in pochi passaggi.',style:TextStyle(color:Colors.white54)),const SizedBox(height:18),_progress(),const SizedBox(height:18),
      if(step==0)_stepProduct() else if(step==1)_stepMode() else _stepPrice(),
    ]));
  }
  Widget _progress()=>Row(children:[for(int i=0;i<3;i++)... [Expanded(child:Container(height:6,decoration:BoxDecoration(color:i<=step?MercDealTheme.green:Colors.white12,borderRadius:BorderRadius.circular(5)))),if(i<2)const SizedBox(width:5)]]);
  Widget _stepProduct()=>Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    GlassCard(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('1. PRODOTTO',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900)),const SizedBox(height:12),Container(height:155,decoration:BoxDecoration(color:MercDealTheme.card2,borderRadius:BorderRadius.circular(18)),child:const Center(child:Icon(Icons.add_photo_alternate_outlined,size:52,color:MercDealTheme.green))),const SizedBox(height:12),TextField(controller:title,decoration:const InputDecoration(labelText:'Titolo annuncio',hintText:'Es. iPhone 14 Pro 128GB')),const SizedBox(height:10),const TextField(maxLines:4,decoration:InputDecoration(labelText:'Descrizione',hintText:'Descrivi condizioni, accessori e dettagli')),const SizedBox(height:10),const Text('Condizioni',style:TextStyle(fontWeight:FontWeight.w900)),const SizedBox(height:6),Wrap(spacing:7,children:[ChoiceChip(label:const Text('Nuovo'),selected:false,onSelected:(_){ }),ChoiceChip(label:const Text('Come nuovo'),selected:true,onSelected:(_){ }),ChoiceChip(label:const Text('Ottime'),selected:false,onSelected:(_){ }),ChoiceChip(label:const Text('Buone'),selected:false,onSelected:(_){ })])])),
    const SizedBox(height:14),GlowButton(label:'Avanti',icon:Icons.arrow_forward_rounded,onPressed:()=>setState(()=>step=1)),
  ]);
